...
// --- basic planted 3-SAT generator (no solver changes) ---
...
int main(){
    ...
    // --- BIG but still "basic" demo ---
    int nVars = 300;             // moderate size to keep baseline solver fast
    int nClauses = 1200;         // ratio ~4.0
    ...
    s.alpha = 0.25;
    s.zeta  = 0.05; // mírný šum

    int solved = TrySolveIncreasing(&s, 50, 1<<17); // až ~131k kroků

    if (solved){
        printf("Final answer reached with energy 0.\n");
        printf("Assignments (first 40 vars):\n");
        for (int i = 0; i < s.nVars && i < 40; ++i){
            printf("  x%-3d = %d%s", i+1, s.assign[i], ((i%10)==9 || i==39 || i==s.nVars-1) ? "\n" : "  ");
        }
    ...
    // --- poly-time coherence witness on current assignment ---
